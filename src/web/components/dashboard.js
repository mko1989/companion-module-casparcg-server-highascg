...
const LAYER_COUNT = 9
// PRV preview uses this layer; setupPreviewChannel uses layer 11 for the PGM route
const PRV_PREVIEW_LAYER = 11
...
export function initDashboard(root, stateStore) {
	...
	/** Play a clip on the PRV channel (layer 11) for preview. PRV only shows explicitly requested content. */
	...
	/** Clear PRV layer 11 — called when user deselects or clicks elsewhere. */
				...
				try {
					if (!sameClip) {
						if (src.type === 'timeline') {
							...
							await api.post('/api/play', { channel: programCh, layer: casparLayer })
						} else {
							// CUT: direct PLAY
					...
					await api.post('/api/mixer/volume', { channel: programCh, layer: casparLayer, volume: ls.volume ?? 1 })
	...
	function render() {
		...
		columns.forEach((col, colIdx) => {
			...
			cell.innerHTML = `
				...
				<div class="dashboard-col-popover" id="popover-col-${colIdx}" hidden>
					...
					<div class="dashboard-popover-row">
						...
						<input type="number" data-field="duration" min="0" value="${trans.duration ?? 0}" />
					</div>
					<div class="dashboard-popover-row">
						<label>Tween</label>
			...
			popover.querySelector('[data-field="duration"]').addEventListener('change', (e) => {
				...
				dashboardState.setColumnTransition(colIdx, { duration: isNaN(v) ? 0 : Math.max(0, v) })
			})
		...
		for (let layerIdx = 0; layerIdx < LAYER_COUNT; layerIdx++) {
			...
			columns.forEach((col, colIdx) => {
				...
				cell.addEventListener('click', (e) => {
					...
					window.dispatchEvent(new CustomEvent('dashboard-select', { detail: { colIdx, layerIdx, cell: layer } }))
					if (src?.value) {
						cuePreview(src)
					} else {
						clearPreview()
	...
	function showToast(msg, type = 'info') {
		...
		toast.style.cssText = `padding:10px 16px;border-radius:6px;font-size:13px;max-width:360px;word-break:break-all;box-shadow:0 2px 8px rgba(0,0,0,.4);background:${type === 'error' ? '#b91c1c' : '#1d4ed8'};color:#fff;`
